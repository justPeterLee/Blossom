...
router.get("/explore/plant/:id", rejectUnauthenticated, (req, res) => {
  if (req.isAuthenticated) {
    ...
    axios
      .get(
        `https://perenual.com/api/species-list?key=${process.env.PERENUAL_KEY}&page=${pageId}`
      )
      .then((response) => {
...
router.get("/explore/garden/:id", rejectUnauthenticated, (req, res) => {
  if (req.isAuthenticated) {
    ...
    axios
      .get(
        `https://perenual.com/api/species-list?key=${process.env.PERENUAL_KEY}&indoor=1&page=${data}`
      )
      .then((response) => {
        const data = response.data;
        let sendData = [];
